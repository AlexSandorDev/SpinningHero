	...
	void DrawIndicators()
	{
		foreach (Target target in targets)
		{
			...
			else if (target.NeedArrowIndicator && !isTargetVisible)
			{
				...
				if (source != null) _center = mainCamera.WorldToScreenPoint(source.position);

				OffScreenIndicatorCore.GetArrowIndicatorPositionAndAngle(ref screenPosition, ref angle, _center, screenBounds);
